...
static UniqueFile Open(const std::filesystem::path& path, const char* mode) {
...
void NsoFile::Dump(bool verbose) {
  char msg[1024];
  char* p = msg;
...
bool NsoFile::Load(const std::vector<u8>& file) {
  ...
  if (file.size() >= sizeof(NsoHeader) &&
    ...
    auto nro = reinterpret_cast<const NroHeader*>(&file[nro_offset]);
  ...
  const u8* mod_base = nullptr;
  const ModPointer* mod_ptr = nullptr;
  if (file_type != kUnknown) {
    ...
    mod_ptr = reinterpret_cast<const ModPointer*>(&file[0]);
  ...
  auto mod = reinterpret_cast<const ModHeader*>(mod_base);
  ...
  auto mod_get_offset = [&](s32 relative_offset) {
    auto ptr = reinterpret_cast<const u8*>(mod_base + relative_offset);
    auto offset = reinterpret_cast<uintptr_t>(ptr) -
                  reinterpret_cast<uintptr_t>(&image[0]);
    return static_cast<u32>(offset);
  };

  dynamic = reinterpret_cast<const Elf64_Dyn*>(mod_base + mod->dynamic_offset);
...
#undef ALLOC_SHDR_IF
  ...
  if (eh.MeasureFrame(reinterpret_cast<eh_frame_hdr*>(&image[eh_info.hdr_addr]),
                      &eh_frame_ptr, &eh_info.frame_size)) {
    eh_info.frame_addr =
        eh_info.hdr_addr +
        (eh_frame_ptr - reinterpret_cast<uintptr_t>(&image[eh_info.hdr_addr]));
